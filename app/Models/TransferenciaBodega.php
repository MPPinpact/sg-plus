...
class TransferenciaBodega extends Authenticatable
{
    /**
    ...
    protected $table = 'transferencia_bodega';

    protected $primaryKey = 'IdTransferencia';

    protected $fillable = [
        'IdTransferencia', 'TipoTransferencia', 'IdEmpresa', 'IdLocal', 'IdBodega', 'IdBodegaDestino', 'FechaTransferencia', 'TotalNeto', 'TotalImpuestos', 'TotalCompra', 'EstadoTransferencia'
    ];
    ...
    public function listTransferencias($TipoTransferencia){
        if($TipoTransferencia==1)
            return DB::table('v_transferencia_bodega')->where('EstadoTransferencia', '=', 1)->get();
        else 
            return DB::table('v_transferencia_bodega')->get();
    }

    public function getTransferenciaAbierta($IdLocal){
        return DB::table('v_transferencia_bodega')->where([['IdLocal','=', $IdLocal],['EstadoTransferencia','=','1']])->orderBy('IdTransferencia','desc')->get();
    }

    public function finalizarCompraMasiva($IdTransferencia){
        $IdUsuario = Auth::id();

        $values=array('EstadoTransferencia'=>2,'auFechaModificacion'=>date("Y-m-d H:i:s"),'auUsuarioModificacion'=>$IdUsuario);
        
        return DB::table('v_transferencia_bodega')->where('IdTransferencia', $IdTransferencia)->update($values);
    }

    // Cargar combo de estados de Estado (Activo / Inactivo)
    public function listEstadosTransferencia(){
        return DB::table('v_estados_transferencia')->get();
    }
    ...
    // registrar compra
    public function regTransferencia($datos){
        $idAdmin = Auth::id();
    ...
    public function regDetalleTransferencia($datos){
        $idAdmin = Auth::id();
